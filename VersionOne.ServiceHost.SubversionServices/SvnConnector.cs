...
namespace VersionOne.ServiceHost.SubversionServices
{
	public class SvnInformation 
    {
        ...
        public SvnInformation(string url, string root, string path) 
        {
            Url = url;
            Root = root;
            Path = path;
    ...
    public class SvnConnector : IDisposable
	{
        ...
        public void SetAuthentication(string username, string password, string ssloverride)
		{
		    client.Authentication.ClearAuthenticationCache();
            client.Authentication.Clear();
            if (Convert.ToBoolean(ssloverride))
        ...
        public SvnInformation GetSvnInformation(string url)
        {
            ReadSvnInformation(url);
            return svnInfo;
        }
        ...
        {
            return Checkout(repoUrl, workingPath, new SvnRevision(SvnRevisionType.Head), recurse, ignoreExternals);
        }
		...
		{
			if (_error != null)
				_error(this, new SvnExceptionEventArgs(ex));
		}
		private event EventHandler<SvnExceptionEventArgs> _error;
		public event EventHandler<SvnExceptionEventArgs> Error
		{
			add { _error += value; }
			remove { _error -= value; }
		}
